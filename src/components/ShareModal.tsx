...
export default function ShareModal({ track, shareType, onClose }: ShareModalProps) {
  ...
  // Auto-error the preview after 12s so skeleton doesn't spin forever
  useEffect(() => {
    if (imgLoaded || imgError) return;
    const t = setTimeout(() => setImgError(true), 12000);
  ...
  async function shareToSocial(platformId: string) {
    ...
    // On mobile, try native share sheet first for Instagram/TikTok (no web share API)
    if (platformId === 'instagram' || platformId === 'tiktok') {
      // Try to share the story image via native share API
      try {
        const res = await fetch(storyUrl);
        if (res.ok) {
          const blob = await res.blob();
      ...
      <div className="share-modal" onClick={e => e.stopPropagation()}>
        ...
        <div className="share-modal-preview-wrap">
          <div className="share-modal-preview">
            ...
            {imgError ? (
              <div className="share-modal-preview-error">
                <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="rgba(201,168,76,0.5)" strokeWidth="1.5">
          ...
          <p className="share-modal-hint">9:16 · Perfect for Instagram & TikTok Stories</p>
        </div>
        ...
        <div className="share-modal-socials">
          {SOCIALS.map(s => (
            ...
            >
              <span className="share-social-icon" style={{ color: s.color }}>
                {s.icon}
              </span>
        ...
        <div className="share-modal-actions">
          ...
          >
            ...
            <span>{downloading ? 'Downloading…' : 'Save Story Card'}</span>
          </button>

          {/* Copy link */}
          <button className="share-modal-btn share-modal-btn--ghost" onClick={copyLink}>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
